...
impl FrameHandler {
    ...
    async fn process_event(
        ...
        match event_handler.do_without_validation(payload.clone()) {
            Ok(_) => {
            ...
            Err(err) => {
                Self::persist_event_error(&err, &payload);
    ...
    fn persist_event_error(err: &anyhow::Error, payload: &[u8]) {
        let log_dir = Path::new(".logs");
        if create_dir_all(log_dir).is_err() {
            return;
        }

        let log_path = log_dir.join("feishu-event-errors.log");
        ...
        let _ = writeln!(
            file,
            "[{timestamp}] Failed to handle event\nerror: {err:?}\npayload:\n{payload_text}\n---"
        );
    }
