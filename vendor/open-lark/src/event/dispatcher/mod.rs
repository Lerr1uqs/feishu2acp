...
impl EventDispatcherHandler {
    ...
    fn emit_with_aliases(&self, event: &str, payload: &[u8]) -> anyhow::Result<()> {
        if self.processor_map.contains_key(event) {
            return self.emit(event, payload);
        }

        // Some IM read-receipt events appear under two near-identical names:
        // `im.message.read_v1` in parts of the SDK/tests and
        // `im.message.message_read_v1` in real websocket traffic.
        // We accept both so runtime dispatch does not depend on which spelling
        // the upstream event source happens to use.
        for alias in event_aliases(event) {
            if self.processor_map.contains_key(*alias) {
                return self.emit(alias, payload);
            }
        }
    ...
    pub fn do_without_validation(&self, payload: Vec<u8>) -> anyhow::Result<()> {
        ...
        self.emit_with_aliases(&handler_name, &payload)
    }
}

fn event_aliases(event: &str) -> &'static [&'static str] {
    match event {
        // Runtime traffic has been observed with the longer `message_read_v1`
        // name, while SDK fixtures use the shorter `read_v1` spelling.
        "p2.im.message.message_read_v1" => &["p2.im.message.read_v1"],
        "p2.im.message.read_v1" => &["p2.im.message.message_read_v1"],
        _ => &[],
    }
}
...
impl EventDispatcherHandlerBuilder {
    ...
    {
        // Register the canonical SDK key here; alias resolution in `emit_with_aliases`
        // handles the alternate runtime spelling.
        let key = "p2.im.message.read_v1".to_string();
        if self.processor_map.contains_key(&key) {
            return Err(format!("processor already registered, type: {key}"));
        }
